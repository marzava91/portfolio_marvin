  ...
  Widget _buildSkillBoxes({
    ...
    return MasonryGridView.count(
      ...
      itemBuilder: (context, index) {
        if (index == 1 || index == 5) {
          return SizedBox(height: invisibleBoxHeight); // sin tileBuilder
        } else {
          final parentWidth = widthOfScreen(context) * 0.4; // solo la mitad del layout
          final spacing = (crossAxisCount - 1) * kCrossAxisSpacing;
          final sidePadding = Sizes.HEIGHT_48 ; // si usas padding horizontal 48
          final cardWidth = (parentWidth - spacing - sidePadding) / crossAxisCount;

          return SizedBox(
            width: cardWidth,
            height: cardWidth, // igual a width → cuadrado
            child: SkillCard(
